...
DEEPFACE_DISTANCE_THRESHOLD = 0.5
                ...
                for box in r.boxes:
                    ...
                    if distance < DEEPFACE_DISTANCE_THRESHOLD:
                        ...
                        if (current_time - self.last_unknown_capture_time) > UNKNOWN_CAPTURE_COOLDOWN:
                                                        ...
                                                        # 1. Get the current datetime for both the filename and the overlay text
                            now = datetime.datetime.now()
                            timestamp_for_display = now.strftime("%Y-%m-%d %H:%M:%S")
                            timestamp_for_filename = now.strftime("%Y%m%d_%H%M%S")

                            # 2. Draw the timestamp text directly onto the 'face' image
                            cv2.putText(face, 
                                        timestamp_for_display, 
                                        (5, 10),  # Position (5px from left, 10px from top)
                            ...
                            filepath = os.path.join(UNKNOWN_FACES_PATH, f"unknown_{timestamp_for_filename}.jpg")
