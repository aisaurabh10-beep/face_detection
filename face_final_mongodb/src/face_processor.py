...
class FaceProcessor:
    ...
    def __init__(self, yolo_model, embeddings_db, names, config):
        ...
        self.names = names
        self.config = config
        self.last_seen_times = {} # In-memory dictionary for attendance cooldown
        self.last_unknown_capture_time = 0
        ...
        self.cooldown = config.getfloat('Performance', 'unknown_capture_cooldown')
        self.unknown_path = config.get('Paths', 'unknown_faces_path')
        self.attendance_cooldown = config.getint('Performance', 'attendance_mark_minutes') * 60



    # # The rest of the FaceProcessor class (process_frame, _save_unknown_face) remains the same
    # def process_frame(self, frame, frame_counter):
    #     # ... no changes here ...
    #     return frame
    ...
    def _recognize_and_draw(self, frame, face, coords, yolo_conf):
        ...
        try:
            rep = DeepFace.represent(img_path=face, model_name="ArcFace", enforce_detection=False)
            if not rep or "embedding" not in rep[0]: return
            ...
            if distance < self.dist_thresh:
                student_id = self.names[best_idx]
                
                # Check in-memory cooldown before calling the API
                current_time = time.time()
                if student_id not in self.last_seen_times or \
                   (current_time - self.last_seen_times.get(student_id, 0)) > self.attendance_cooldown:
                    
                    if post_attendance(student_id, yolo_conf, distance, self.config):
